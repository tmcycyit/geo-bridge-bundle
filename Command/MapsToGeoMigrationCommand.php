...
//                        $connection->executeUpdate("CALL GeoDataMigration('{$table['name']}', '$databaseName', '$columnName')");
                        ...
                        if($databaseName == 'maps' && isset($exist['result']) && $exist['result'] > 0){
								// get addresses id`s in project
								$addressCompany = " SELECT  geo_" . $columnName . " , SUBSTRING_INDEX(  `coordinates` ,  ',', 1 ) , SUBSTRING_INDEX(  `coordinates` ,  ',', -1 )
        ...
        foreach ($idsResults as $idResult) {
            ...
            foreach ($idResult as $ids) {
				...
				$ladit = $this->produceUrlParameter($ids["SUBSTRING_INDEX(  `coordinates` ,  ',', 1 )"]);
				$lodit = $this->produceUrlParameter($ids["SUBSTRING_INDEX(  `coordinates` ,  ',', -1 )"]);
				$context  = stream_context_create($opts);

				$addresses = $this->getContent(self::GEO_DOMAIN . "api/put/address/".$hNumber."/".$ladit."/".$lodit , $context);
				$address = $ids['geo_address'];
				if(isset($addresses) && $addresses != null && isset($address) && $address != null)
				{
					$connection->executeUpdate("CALL GeoDataModified($addresses , '$address')");
				}
...
//						if(is_null($address)){
////							$addresses = $this->getContent(self::GEO_DOMAIN . 'api/put/address/'Addresss 23'/'40.20888474177205'/'44.5166601402282 '');
//
								...
								SET mainColumn = SUBSTRING(columnName, 5);
                                    ...
                                    BEGIN
										SET @update = CONCAT('UPDATE ',tableName,' SET ',mainColumn,' = (
																SELECT id FROM yit_geo_address WHERE address = ', columnName,');');
										PREPARE stmt FROM @update ;
										EXECUTE stmt;
                                    END;
                                    BEGIN
